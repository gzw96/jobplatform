            ...
            <li class="list-group-item">
                <div style="height: 50px">
                    ...
                    <div style="width: 89%;float: left">
                        <a href="/forum/t/${pageInfo.list.get(status.index).id}">${pageInfo.list.get(status.index).title}</a><br/>
                        <div>
                            <a><span class="label label-default" >${pageInfo.list.get(status.index).tab.tabName}</span></a>&nbsp;&nbsp;&nbsp;
                            <a href="/forum/member/${pageInfo.list.get(status.index).user.username}"><span ><strong>${pageInfo.list.get(status.index).user.username}</strong></span></a>&nbsp;&nbsp;&nbsp;
    ...
    <script>
        ...
        $('[data-page]').on('click', function () {
            ...
            if (window.location.pathname.indexOf("tab")<0){//判断是否是tab页面
                window.location.href = encodeURI('/forum/${topic.id}?pageNum=' + pageNum);
            }else {
                window.location.href = encodeURI('/forum/tab/${tab.tabNameEn}?pageNum=' + pageNum);
            }
        });
        //点击上一页
